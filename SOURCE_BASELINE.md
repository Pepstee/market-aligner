...
`python -m baseline_adoption.cli publish-evidence --receipt <receipt> --data-root <runtime> --repository <repository>` publishes `runtime_evidence/JAA-00-online-snapshot.yaml`. It first completes independent review, so failure leaves existing evidence untouched; success stages, validates, and atomically replaces it. The evidence binds the receipt's capture commit, the exact current non-generated source content and inventory, SQLite snapshots, dependency-record hashes, Python runtime and distributions, host prerequisites, preservation, rollback, and symbolic secret references. The capture commit must be an ancestor of the reviewed checkout; a later commit is accepted only while the independently hashed non-generated source bytes and inventory still match. This avoids making the tracked generated evidence self-invalidating when it is committed. The evidence contains no secret values or paths, declares no volatile fields, and is byte-stable while verified inputs are unchanged.
