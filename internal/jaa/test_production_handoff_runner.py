...
def test_public_runner_owns_current_time_and_exposes_no_release_path(monkeypatch, tmp_path: Path) -> None:
    ...
    monkeypatch.setattr(runner, "installed_production_handoff_deployment", lambda: deployment)
    ...
    assert "deployment" not in inspect.signature(runner.run_production_handoff).parameters
    ...
    assert observed["subject_sha256"] == hashlib.sha256(
        canonical_json_bytes(expected_subject)
    ).hexdigest()
    assert observed["builder"]["freshness_time"].isoformat() == "2026-08-21T00:01:00+00:00"


@pytest.mark.parametrize("field", ["data_home", "output_root", "repository_root"])
def test_alternate_roots_fail_before_time_or_state_read(
    ...
    monkeypatch.setattr(runner, "installed_production_handoff_deployment", rejected_deployment)
    monkeypatch.setattr(runner, "installed_production_current_time_witness", forbidden_time)
    monkeypatch.setattr(runner, "_build_production_handoff_from_authenticated_time", forbidden_build)
    with pytest.raises(runner.ProductionHandoffDeploymentError, match="compiled canonical roots"):
...
def test_compiled_deployment_document_is_exact_and_receipt_root_is_derived() -> None:
    ...
    assert runner._parse_deployment_configuration(exact) == hashlib.sha256(exact).hexdigest()
    assert runner.PRODUCTION_MARKET_EXECUTION_RECEIPT_ROOT == (
        runner.PRODUCTION_MARKET_OUTBOX_ROOT / "receipts"
    )


def test_outbox_symlink_is_rejected_before_time_or_state(monkeypatch, tmp_path: Path) -> None:
    data_home = tmp_path / "data"
    repository_root = tmp_path / "repo"
    ...
    with pytest.raises(runner.ProductionHandoffDeploymentError, match="outbox identity"):
        runner.run_production_handoff(
            profile_id="prf_" + "1" * 32,
            track="software-engineering",
            source_job_key="workable:cogna:847CFBC5F4",
