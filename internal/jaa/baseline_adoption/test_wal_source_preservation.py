...
def test_recertification_rejects_checkpoint_during_final_wal_observation(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch,
) -> None:
    """The final whole-source scan must itself remain inside the mutation boundary."""
    source_root = tmp_path / "source"
    source = source_root / "inputs" / "jobs.sqlite3"
    writer = _make_live_wal(source)
    wal = Path(str(source) + "-wal")
    try:
        ...
        def checkpoint_before_third_wal_hash(candidate: Path) -> str:
            nonlocal wal_hashes
            if candidate == wal:
                wal_hashes += 1
                if wal_hashes == 3:
        ...
        monkeypatch.setattr(core, "_hash_file", checkpoint_before_third_wal_hash)

        with pytest.raises(core.AdoptionError, match="mutation boundary"):
            core._recertify_source(source, spec)
