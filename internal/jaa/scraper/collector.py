...
from contracts import JobUrl, RawPosting, write_jsonl
from scraper.adapters.base import SourceUnavailable, load_adapter
from scraper.database import JobDatabase
from scraper.scrapling_client import ScraplingClient, ScraplingFetchError
...
def _save_raw(base: Path, row: RawPosting) -> None:
    write_jsonl(_raw_path(base, row), [row])


class Collector:
    def __init__(self, cfg: dict[str, Any], root: Path, log=print) -> None:
        self.cfg, self.root, self.log = cfg, root, log
        io = cfg.get("io", {}) or {}
        self.urls_path = root / io.get("job_urls", "scraper/data/job_urls.jsonl")
        self.raw_cache = root / io.get("raw_cache", "scraper/data/raw_cache")
        self.db = JobDatabase(root / io.get("database", "scraper/data/jobs.sqlite3"))
        ...
        self.scrapling = ScraplingClient(root, scrapling) if scrapling.get("enabled", False) else None
