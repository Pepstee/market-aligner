    ...
    if matched:
        ...
        for requirement, (_requirement_id, _text, claim_type) in zip(
            ...
            graph.add_evidence(
                ...
                valid_until=(
                    _utc_today().replace(year=_utc_today().year + 1).isoformat()
                ),
            )
        ...
        evidence = candidate_graph_evidence(database.path, as_of=_utc_today())
...
def test_explicit_eligible_gap_policy_preserves_gaps_and_emits_only_supported_directives() -> None:
    supported = _requirement("supported")
    uncovered = _requirement("uncovered", essential=False)
    strategy = compile_application_strategy(
        fit_run_id=DIGEST,
        dossier_hash=hashlib.sha256(b"dossier").hexdigest(),
        candidate_profile_hash=hashlib.sha256(b"profile").hexdigest(),
        requirements=(supported, uncovered),
        match_results=(
            _matched(supported),
            MatchResult(
                ...
                (),
                10_000,
                "no approved evidence match",
                POLICY.policy_hash,
                None,
            ),
        ),
        candidate_support=(_support(supported),),
        employer_facts=(_fact(),),
        as_of=AS_OF,
        permit_eligible_gap_application=True,
    )
    assert strategy.decision == "apply_with_known_gaps"
    assert [row.state for row in strategy.coverage] == ["covered", "absent"]
    assert len(strategy.elements) == len(ELEMENT_KINDS)
    assert {row.requirement_id for row in strategy.elements} == {
        supported.requirement_id
    }
...
def test_ready_fit_compiles_persists_and_routes_strategy_atomically(
    ...
    assert store.load(strategy.strategy_id, as_of=_utc_today()) == strategy
...
def test_candidate_graph_drift_after_fit_suppresses_strategy_receipt(
    ...
    graph.add_evidence(
        ...
        state="evidence",
        evidence_kind="portfolio_artifact",
        valid_until=_utc_today().replace(year=_utc_today().year + 1).isoformat(),
    )
    graph.verify_evidence(
        "later-evidence",
        1,
    ...
    with pytest.raises(ValueError, match="candidate graph changed"):
        store.compile_and_record(fit_run_id=run.run_id, as_of=_utc_today())
    with store._connect() as connection:
        assert connection.execute(
...
def test_changed_strategy_retry_conflicts_without_mutating_original(
    ...
    with pytest.raises(IdempotencyConflict):
        store.compile_and_record(
            fit_run_id=run.run_id,
            as_of=_utc_today() + timedelta(days=1),
        )
    with store._connect() as connection:
        persisted = connection.execute(
...
def test_offline_strategy_element_tamper_fails_durable_read(
    ...
    with pytest.raises(ValueError, match="kind and directive"):
        store.load(strategy.strategy_id, as_of=_utc_today())
