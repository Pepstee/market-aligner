...
def _state_fingerprint(state: Mapping[str, Any]) -> str:
    # Socket tables can legitimately transition (for example TIME_WAIT expiry)
    # between the worker's inner capture and the supervisor's outer capture.
    # The supervisor records and signs its own exact pre/post socket-table hashes,
    # while descriptor checks prove that the worker retains no socket.  Bind only
    # temporally stable namespace state across this asynchronous handshake.
    selected = {
        key: state[key]
        for key in (
            "network_namespace",
