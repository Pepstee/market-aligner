...
class ReleaseBinding:
    ...
    def __post_init__(self) -> None:
        ...
        for value, label in (
            ...
            _digest(value, label)
        if len(set(self.model_receipt_sha256s)) != len(self.model_receipt_sha256s):
...
class ValidationReceipt:
    ...
    def __post_init__(self) -> None:
        ...
        if len(set(self.finding_codes)) != len(self.finding_codes) or any(
            not value.strip() for value in self.finding_codes
        ):
    ...
    def _questions_document(
        ...
        for requirement_id, pair in sorted(values.items()):
            ...
            if (
                ...
                raise ValueError(
                    "portal question binding must be two non-empty strings"
                )
    ...
    def register(
        ...
        *,
        ...
        if expected_artifacts != artifacts:
            raise ValueError(
                "application artifacts differ from deterministic rendering"
            )
        ...
        try:
            ...
            transition = self.lifecycle.commit_in_transaction(
                ...
                outputs={
                    ...
                    "artifact_relative_directory": (publication.relative_directory),
                },
                idempotency_key=(
            ...
            document_json = canonical_json(compilation.document(include_receipt=False))
            expected = (
...
class IssuedRelease:
    ...
    def __post_init__(self) -> None:
        ...
        if hashlib.sha256(self.release_token.encode()).hexdigest() != self.token_sha256:
...
class ReleaseGateStore:
    ...
    POLICY_SHA256 = hashlib.sha256(
        canonical_json(
            {
                "contract": "jaa08.release-gate.v2",
                "validators": REQUIRED_VALIDATORS,
                "external_document_assurance_policy_sha256": (ASSURANCE_POLICY_SHA256),
    ...
    VALIDATOR_CONTRACTS = {
        ...
        "ats": (
            "strict-parse-reconstruct-and-assure-employer-facing-"
            "one-or-two-page-cv-one-page-letter-and-answers"
        ),
        ...
        *,
        ...
        for row in rows:
            ...
            for name in ("issued_at", "consumed_at"):
                ...
                if parsed.tzinfo is None or parsed.utcoffset() is None:
                    raise ValueError("stored release timestamp must include a timezone")
    ...
    def _binding(
        ...
        *,
        ...
        if (
            ...
            or str(row["application_source_id"]) != compilation.application_source_id
            or str(row["application_source_hash"])
            != compilation.application_source_sha256
            or str(row["artifact_set_hash"]) != compilation.artifact_set_sha256
            or str(row["artifact_receipt_hash"]) != compilation.artifact_receipt_sha256
        ...
        candidate_identity = content_hash(
            {
                ...
                "candidate_profile_sha256": str(row["candidate_profile_hash"]),
            }
        )
        prior_count = int(
            connection.execute(
    ...
    def _validate_authority(
        ...
        if (
            ...
            or compilation.application_source_id != binding.application_source_id
            ...
            or publication.receipt_sha256 != binding.artifact_receipt_sha256
        ):
    ...
    def _validate_consistency(
        ...
        if (
            ...
            or artifacts.artifact_set_sha256 != binding.artifact_set_sha256
            or compilation.artifact_set_sha256 != binding.artifact_set_sha256
            or publication.artifact_set_sha256 != binding.artifact_set_sha256
            or compilation.artifact_receipt_sha256 != publication.receipt_sha256
        ):
    ...
    def _validate_official_route(binding: ReleaseBinding) -> None:
        ...
        if (
            ...
            raise ValueError("official-route validator found no current allowed route")
    ...
    def _finding_code(cls, error: Exception) -> str:
        ...
        if "external-document assurance" in message:
            return "external_document_assurance"
    ...
    def _record_blocked_attempt(
        ...
        *,
        ...
        try:
            ...
            if (
                state is None
                or str(state["state"]) != PipelineState.APPLICATION_COMPILED.value
            ):
                connection.rollback()
                return
            sequence = (
            ...
            transition = self.lifecycle.commit_in_transaction(
                ...
                idempotency_key=(f"release-blocked:{compilation.job_key}:{attempt_id}"),
            )
            connection.execute(
    ...
    def evaluate_and_issue(
        ...
        *,
        ...
        try:
            ...
            if existing is not None:
                try:
                    ...
                    error.add_note(
                        f"blocked-attempt audit failed: {type(audit_error).__name__}"
                    )
            raise
        if compilation.compilation_id != compilation_id:
            error = ValueError("release cites a different compilation identity")
            try:
                self._record_blocked_attempt(
        ...
        try:
            ...
            token = (
                f"jaa08.{manifest.release_manifest_sha256}.{secrets.token_urlsafe(32)}"
            )
    ...
    def _stored_compilation(row: sqlite3.Row) -> ApplicationCompilation:
        ...
        try:
            ...
            raise ValueError("stored application compilation is invalid JSON") from exc
        if (
            not isinstance(document, dict)
            or canonical_json(document) != str(row["compilation_document_json"])
            or document != compilation.document(include_receipt=False)
        ):
            raise ValueError("stored application compilation differs from its identity")
        return compilation
    ...
    def _stored_manifest_document(
        ...
        if not isinstance(document, dict) or canonical_json(document) != str(
            row["manifest_document_json"]
        ):
    ...
    def _verify_stored_validations(
        ...
        for row, expected in zip(rows, manifest.validations, strict=True):
            try:
                ...
                raise ValueError("stored release validation is invalid JSON") from exc
            if (
                str(row["validator_id"]) != expected.validator_id
                or str(row["validator_version"]) != expected.validator_version
                or str(row["validator_impl_hash"]) != expected.validator_impl_sha256
                or str(row["input_hash"]) != expected.input_sha256
                or str(row["artifact_set_hash"]) != expected.artifact_set_sha256
                or str(row["decision"]) != expected.decision
                or canonical_json(document) != str(row["receipt_document_json"])
                or canonical_json(document) != canonical_json(expected.document())
            ):
                raise ValueError(
    ...
    def verify_token_official_route(
        ...
        *,
        ...
        """Rebind a token to the exact provider route before browser use."""
        if not release_token.startswith("jaa08.") or len(release_token.split(".")) != 3:
            raise ValueError("release token format is invalid")
        ...
        try:
            ...
            stored = self._stored_manifest_document(row)
        finally:
            connection.close()
        try:
            ...
            if not isinstance(binding, dict):
                raise TypeError
            route_document = binding["official_route"]
            if not isinstance(route_document, dict):
                raise TypeError
            route = OfficialRouteBinding(
                route_id=str(route_document["route_id"]),
                adapter_id=str(route_document["adapter_id"]),
                adapter_version=str(route_document["adapter_version"]),
                source_identity=str(route_document["source_identity"]),
                route_policy_sha256=str(route_document["route_policy_sha256"]),
                verified_at=date.fromisoformat(str(route_document["verified_at"])),
                valid_until=date.fromisoformat(str(route_document["valid_until"])),
                allowed=bool(route_document["allowed"]),
            )
        except (KeyError, TypeError, ValueError) as exc:
            raise ValueError("stored official route binding is malformed") from exc
        if (
            route.document() != route_document
            or route.adapter_id != adapter_id
            or route.adapter_version != adapter_version
            or route.source_identity != source_identity
            or not route.allowed
        ):
            raise ValueError("release token cites a different official route")
        return route
    ...
    def _verify_release_replay(
        ...
        *,
        ...
        if release_token.split(".")[1] != str(row["release_manifest_hash"]):
            raise ValueError("release token cites a different manifest identity")
        ...
        try:
            issued_date = date.fromisoformat(str(binding_document["evaluated_at"]))
        except (KeyError, TypeError, ValueError) as exc:
            raise ValueError("stored release clock is invalid") from exc
        if consumed_utc.date() != issued_date:
            raise ValueError("release token is valid only on its evaluated UTC date")
        ...
        if current_binding.prior_application_count != 1:
            raise ValueError("release identity has an unexpected application count")
        ...
        if canonical_json(replay_manifest.document()) != canonical_json(
            ...
            raise ValueError("release token was invalidated by bound-input drift")
