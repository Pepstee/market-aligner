...
PLAN_DECISIONS = frozenset(
    {
        "apply_now",
        "apply_with_known_gaps",
        "close_gap_first",
        "reject_candidacy",
    }
)
LEGACY_PLAN_DECISIONS = frozenset(
    {"apply_now", "close_gap_first", "reject_candidacy"}
)
APPLICATION_DECISIONS = frozenset({"apply_now", "apply_with_known_gaps"})
...
class ApplicationStrategy:
    ...
    def __post_init__(self) -> None:
        ...
        if self.decision in APPLICATION_DECISIONS:
            if self.decision == "apply_now" and any(
                row.state != "covered" for row in self.coverage
            ):
                raise ValueError("apply-now strategy must cover every requirement")
            if self.decision == "apply_with_known_gaps" and all(
                row.state == "covered" for row in self.coverage
            ):
                raise ValueError(
                    "known-gap strategy requires at least one uncovered requirement"
                )
            covered = tuple(row for row in self.coverage if row.state == "covered")
            expected = len(covered) * len(ELEMENT_KINDS)
            if len(self.elements) != expected:
                raise ValueError(
                    "application strategy has incomplete covered-requirement directives"
                )
            ...
            coverage_by_id = {row.requirement_id: row for row in covered}
...
def compile_application_strategy(
    *,
    ...
    has_gap = any(row.state != "covered" for row in coverage)
    decision = (
        "apply_with_known_gaps"
        if has_gap and permit_eligible_gap_application
        else "reject_candidacy"
        if structural_block
        else "close_gap_first"
        if has_gap
        else "apply_now"
    )
    if decision in APPLICATION_DECISIONS and not fact_rows:
        raise ValueError(
            "application strategy requires current source-backed employer facts"
        )
    support_document = [
        {
            **{
    ...
    if decision in APPLICATION_DECISIONS:
        covered_requirements = tuple(
            requirement
            for requirement in requirement_rows
            if result_by_id[requirement.requirement_id].decision == "matched"
        )
        for index, requirement in enumerate(covered_requirements):
            result = result_by_id[requirement.requirement_id]
