...
_SEMANTIC_REJECTIONS: dict[IntelligenceKind, tuple[str, ...]] = {
    ...
    IntelligenceKind.ROLE: (
        r"\brole in (?:the )?(?:town|history|society|culture)\b",
        r"\bexhibition\b",
    ),
    IntelligenceKind.PRODUCT: (r"\bdiscontinued\b", r"\bsouvenir\b"),
    IntelligenceKind.HIRING: (
        r"\balumni hired\b",
        r"\bhired a speaker\b",
        r"\bnot a vacancy\b",
    ),
    IntelligenceKind.OPERATIONAL_HEALTH: (
        r"\bcolou?r current\b",
        r"\bwithout financial results\b",
    ),
}
...
ATS_AUTHORITY_CANARIES = (
    ATSAuthorityCanary(
        "greenhouse:anthropic:5030244008",
        "Anthropic",
    ...
    ATSAuthorityCanary(
        "ashby:lendable:043d9c49-43e6-4a27-ad55-12344a941974",
        "Lendable",
...
class ATSRouteAdapter:
    ...
    def _identity(self, task: Any) -> tuple[str, str]:
        ...
        if len(parts) != 3 or parts[0].casefold() != self.family:
            raise ValueError(
                "ATS job key is not a canonical family:tenant:vacancy identity"
            )
        tenant, vacancy = parts[1], parts[2]
        if (
            not tenant
            or not vacancy
            or any(value in {".", ".."} for value in (tenant, vacancy))
        ):
            raise ValueError(
                "ATS tenant and vacancy identities must be non-empty path segments"
            )
    ...
    def _validate_route(self, task: Any, url: str, *, final: bool = False) -> None:
        ...
        if not valid:
            raise ValueError(
                f"{self.family} route does not match the admitted tenant and vacancy identity"
            )
    ...
    def authority_url(self, task: Any) -> str:
        ...
        if self.family == "greenhouse":
            ...
            if path != expected:
                raise ValueError(
                    "Greenhouse admitted path does not match its typed identity"
                )
            return f"https://boards-api.greenhouse.io/v1/boards/{tenant}/jobs/{vacancy}"
        if self.family == "ashby":
            if path != f"/{tenant}/{vacancy}":
                raise ValueError(
                    "Ashby admitted path does not match its typed identity"
                )
            return str(task.url)
        if self.family == "lever":
            if path != f"/{tenant}/{vacancy}":
                raise ValueError(
                    "Lever admitted path does not match its typed identity"
                )
            return str(task.url)
        if self.family == "smartrecruiters":
            ...
            if (
                ...
                or not (segments[1] == vacancy or segments[1].startswith(vacancy + "-"))
            ):
                raise ValueError(
                    "SmartRecruiters admitted path does not match its typed identity"
                )
        ...
        if self.family == "workable":
            if path.casefold() != f"/j/{vacancy}".casefold():
                raise ValueError(
                    "Workable admitted path does not match its typed identity"
                )
    ...
    def validate_capture(
        self, task: Any, citation: Citation, cache: "RawResponseCache"
    ) -> None:
        ...
        company_tokens = [
            token
            for token in re.findall(r"[a-z0-9]+", str(task.company).casefold())
            if len(token) > 2
        ]
        title_tokens = [
            token
            for token in re.findall(r"[a-z0-9]+", str(task.title).casefold())
            if len(token) > 3
        ]
        ...
        if not title_tokens or sum(token in text for token in title_tokens) < min(
            2, len(title_tokens)
        ):
            raise ValueError(
                "ATS response does not identify the admitted vacancy title"
            )
...
class ScraplingPublicRetriever:
    ...
    def _terminal_challenge(response: Mapping[str, Any]) -> str | None:
        ...
        markers = (
            ...
            r"\bcaptcha challenge\b",
            r"\bg-recaptcha\b",
            r"\bhcaptcha\b",
            r"\bverify you are human\b",
            r"<title[^>]*>\s*access denied\b",
            r"\bcloudflare challenge\b",
            r"\bcf-chl-",
        )
        ...
        for engine, kwargs in stages:
            ...
            if final_host != requested_host or redirect_hosts - {requested_host}:
                raise PublicAccessDenied(
                    "cross-host redirects are forbidden in JAA public retrieval"
                )
            if challenge := self._terminal_challenge(response):
                raise PublicAccessDenied(f"ACCESS_DENIED: {challenge}")
            if 200 <= status < 300 and len(raw) >= 128 and not renderer_shell:
...
class PortableAuthorityRetriever:
    ...
    def __init__(
        ...
        cache: RawResponseCache,
        *,
        retriever: ScraplingPublicRetriever | None = None,
        maximum_routes: int = 12,
        exact_canaries: bool = True,
        ats_adapters: Mapping[str, ATSRouteAdapter] | None = None,
    ) -> None:
        self.cache = cache
    ...
    def _canary_for(task: Any) -> ATSAuthorityCanary | None:
        ...
        if record is None:
            if seed_host in _TYPED_ATS_HOSTS:
                raise ValueError(
                    "ATS vacancy is outside the exact admitted authority canaries"
                )
            return None
        if (
            str(task.company) != record.company
            or str(task.title) != record.title
            or str(task.url) != record.admitted_url
        ):
            raise ValueError(
                "ATS vacancy identity differs from its exact admitted record"
            )
        return record

    @staticmethod
    def _validate_canary_capture(
        record: ATSAuthorityCanary,
        citation: Citation,
        ...
        *,
        ...
        body = (
            artifact.resolve(citation.raw_response_ref, citation.content_sha256)
            if isinstance(artifact, RawResponseCache)
            else artifact
        )
        if hashlib.sha256(body).hexdigest() != citation.content_sha256:
            raise ValueError(
                "ATS authority bytes differ from the cited SHA-256 artifact"
            )
        requested = citation.requested_url
        if not requested:
        ...
        route_urls = [
            requested,
            *(str(row.get("url", "")) for row in citation.redirect_history),
            citation.url,
        ]
        for route_url in route_urls:
            ...
            if (
                (route.hostname or "").casefold() not in record.authority_hosts
                or route.path not in allowed_paths
            ):
                raise ValueError(
                    "ATS route differs from the admitted tenant or vacancy"
                )
        parsed = urlparse(citation.url)
        if (
            (parsed.hostname or "").casefold() not in record.authority_hosts
        ...
        if (
            ...
            raise ValueError(
                "assets and application forms are not ATS vacancy authority"
            )
        text = _plain_excerpt(body.decode("utf-8", "strict")).casefold()
        if record.company.casefold() not in text or record.title.casefold() not in text:
            raise ValueError(
                "ATS authority bytes do not identify the admitted employer and vacancy"
            )
        if require_temporal:
            published, updated, evidence = extract_publisher_timestamps(body)
            if (published is None and updated is None) or evidence is None:
                raise ValueError(
                    "ATS authority bytes lack an unambiguous publisher date"
                )
            cited_published = (
                _iso_publisher_time(citation.published_at)
                if citation.published_at
                else None
            )
            cited_updated = (
                _iso_publisher_time(citation.updated_at)
                if citation.updated_at
                else None
            )
            if (citation.published_at is not None and cited_published is None) or (
                citation.updated_at is not None and cited_updated is None
            ):
                raise ValueError("ATS publisher date is not verifiable")
            cited_evidence = citation.publisher_date_evidence
            if (
                cited_published != published
                or cited_updated != updated
                or cited_evidence != evidence
            ):
                raise ValueError(
                    "ATS publisher dates differ from the cited response bytes"
                )
    ...
    def _classify(url: str, vacancy_url: str, body: bytes) -> set[IntelligenceKind]:
        ...
        kinds = {
            ...
            if any(
                re.search(pattern, plain.casefold())
                for group in _SEMANTIC_PATTERNS[kind]
    ...
    def _employer_bound(company: str, body: bytes) -> bool:
        ...
        tokens = [
            ...
            if len(token) > 2
            and token not in {"limited", "ltd", "plc", "group", "company"}
        ]
    ...
    def _vacancy_bound(task: Any, citation: Citation, body: bytes) -> bool:
        ...
        text = _plain_excerpt(body.decode("utf-8", "strict")).casefold()
        if re.search(
            r"\b(?:job|vacancy|position) (?:is )?(?:closed|expired|removed|no longer available)\b",
            text,
        ):
            return False
        ...
        return PortableAuthorityRetriever._employer_bound(str(task.company), body) and (
            not title
            or (
                bool(title_tokens)
                and sum(token in text for token in title_tokens)
    ...
    def _aggregator_vacancy_bound(task: Any, citation: Citation, body: bytes) -> bool:
        ...
        try:
            ...
            requested = urlparse(
                _canonical_public_url(citation.requested_url or citation.url)
            )
        except ValueError:
            return False
        if (
            (final.hostname or "").casefold() in AGGREGATOR_HOSTS
    ...
    @staticmethod
    def _source_types(
        item: Citation, vacancy: Citation, body: bytes
    ) -> tuple[str, ...]:
        ...
        if re.search(r"/(?:career|careers|job|jobs|vacanc)", path) or re.search(
            r"\b(?:careers?|vacanc(?:y|ies)|apply)\b", text
        ):
            types.append("official_careers")
        if re.search(
            r"/(?:product|products|service|services|platform|docs?)", path
        ) or re.search(r"\b(?:product|service|platform|documentation)\b", text):
            types.extend(("official_product", "official_product_documentation"))
        if re.search(
            r"/(?:investor|financial|results?|reports?|filings?)", path
        ) or any(re.search(pattern, text) for pattern in _OPERATIONAL_SUBSTANCE):
            types.append("official_financial")
        return tuple(dict.fromkeys(types))

    def retrieve_plan(self, task: Any) -> tuple[list[Citation], list[dict[str, Any]]]:
        canary = (
            self._canary_for(task)
            if self.exact_canaries
            else _ATS_CANARY_BY_KEY.get(str(task.job_key))
        )
        if canary is not None:
            seed = self._retrieve(
                f"source:{task.job_key}:admitted", task.url, "official_vacancy"
            )
            self._validate_canary_capture(
                canary,
                seed,
                self.cache,
            ...
            if canary.authority_url == task.url:
                ...
                authority = self._retrieve(
                    f"source:{task.job_key}:typed-authority",
                    canary.authority_url,
                    "official_vacancy",
                )
        ...
        vacancy = self._retrieve(
            f"source:{task.job_key}:vacancy",
            task.url,
            "discovery_input" if discovery_only else "official_vacancy",
        )
        citations = [] if discovery_only else [vacancy]
        vacancy_body = self.cache.resolve(
            vacancy.raw_response_ref, vacancy.content_sha256
        )
        if not discovery_only and not self._vacancy_bound(task, vacancy, vacancy_body):
            raise ValueError(
                "direct vacancy response is stale, mismatched, empty, or ambiguous"
            )
        ...
        for index, route in enumerate(routes[: self.maximum_routes]):
            ...
            route_kind = (
                "official_vacancy"
                if (
                    route_host in _OFFICIAL_ATS_HOSTS
                    or re.search(r"/(?:job|jobs|vacanc|position|posting)", route_path)
                )
            ...
            try:
                item = self._retrieve(
                    f"source:{task.job_key}:discovered:{index}", route, route_kind
                )
        ...
        if discovery_only:
            if (
                unresolved_official_routes
                or len(official_candidates) != 1
            ...
            citations = [
                item for item in citations if item.source_kind != "official_vacancy"
            ]
            citations.insert(0, vacancy)
        return self._plan_from_citations(task, citations, vacancy)

    def _plan_from_citations(
        self, task: Any, citations: list[Citation], vacancy: Citation
        ...
        for item in citations:
            body = self.cache.resolve(item.raw_response_ref, item.content_sha256)
            if item.id != vacancy.id and not self._employer_bound(
                str(task.company), body
            ):
            ...
            for excerpt, _ in excerpts:
                ...
                for kind in self._classify(
                    item.url, vacancy.url, excerpt.encode("utf-8")
                ):
                    for source_type in self._source_types(item, vacancy, body):
                        ...
                        if (
                            ...
                            and byte_range
                            not in admitted_ranges.setdefault(item.id, set())
                            and _kind_relevant(
                                kind,
                                excerpt,
                                {
...
class SidecarAuthorityRetriever:
    ...
    def retrieve_plan(self, task: Any) -> tuple[list[Citation], list[dict[str, Any]]]:
        ...
        if (
            record.get("vacancy_url") != task.url
            or record.get("company") != task.company
            or record.get("role") != task.title
        ):
            raise ValueError(
                "claimed vacancy identity differs from the frozen authority record"
            )
        ...
        for specification in record["sources"]:
            ...
            if source_type not in production_types[kind]:
                raise ValueError(
                    "authority source type cannot support its assigned purpose"
                )
            ...
            publisher = (
                str(specification.get("canonical_publisher", "")).casefold().strip()
            )
            ...
            if (
                not publisher
                or (
                    publisher != requested_host
                    and not requested_host.endswith("." + publisher)
                )
                or not article
                or article.casefold() in seen_articles
            ):
                raise ValueError(
                    "source is not an independent publisher-owned authority"
                )
            ...
            if publisher != final_host and not final_host.endswith("." + publisher):
                raise ValueError(
                    "redirect escaped the reviewed authoritative publisher"
                )
            if article != final_url:
                raise ValueError(
                    "reviewed canonical article differs from the retrieved final URL"
                )
            if final_url in seen_urls or digest in seen_hashes:
                raise ValueError(
                    "duplicate authority capture cannot support another purpose"
                )
            seen_urls.add(final_url)
            seen_hashes.add(digest)
            seen_articles.add(article.casefold())
            text = re.sub(
                r"\s+", " ", re.sub(r"<[^>]+>", " ", body.decode("utf-8", "strict"))
            ).casefold()
            company_terms = [
                part
                for part in re.findall(r"[a-z0-9]+", str(task.company).casefold())
                if len(part) > 2
            ]
            if company_terms and not any(term in text for term in company_terms):
                raise ValueError(
                    "captured authority does not identify the admitted employer"
                )
            published_at, updated_at, date_evidence = extract_publisher_timestamps(body)
            history = [
                {"url": str(item["url"]), "status_code": int(item["status"])}
            ...
            if specification.get("requires_current") is True and not (
                published_at or updated_at
            ):
                raise ValueError(
                    "freshness-sensitive authority has no verified publisher date"
                )
            ...
            plan.append(
                {
                    ...
                    **(
                        ...
                        if specification.get("excerpt_sha256")
                        else {}
                    ),
                }
            )
...
def extract_publisher_timestamps(
    body: bytes,
    ...
    for match in tag_pattern.finditer(text):
        attrs = {
            key.casefold(): value
            for key, _, value in attr_pattern.findall(match.group(0))
        }
        key = (
    ...
    evidence = next(
        (
            raw
            for field, value, raw in candidates
            if field == chosen_field and value == (updated or published)
        ),
        None,
    )
...
def validate_dossier(
    ...
    *,
    ...
    for source in sources:
        ...
        if (
            citation.id in by_id
            or citation.status_code < 200
            or citation.status_code >= 300
        ):
        ...
        if identity in identities or identity[0] in urls or identity[1] in bodies:
            raise ValueError(
                "URL aliases or duplicate response bodies are not independent evidence"
            )
        ...
        if authority_contract:
            ...
            if not citation.source_kind or not publisher or not article:
                raise ValueError(
                    "source lacks classified kind or canonical publisher/article identity"
                )
            ...
            if article != _canonical_public_url(citation.url):
                raise ValueError(
                    "canonical article identity differs from the captured final URL"
                )
            if article.casefold() in articles:
                raise ValueError(
                    "translations, mirrors, or aliases resolve to one canonical article"
                )
            articles.add(article.casefold())
            plain = re.sub(rb"<[^>]+>", b" ", body)
            normalized = (
                re.sub(rb"\s+", b" ", plain)
                .strip()
                .decode("utf-8", "strict")
            ...
            if equivalence_hash in equivalent_content:
                raise ValueError(
                    "mirrored, syndicated, or repeated content is not independent evidence"
                )
            ...
            if citation.publisher_date_evidence is not None:
                ...
                if evidence not in body:
                    raise ValueError(
                        "publisher date evidence does not resolve to captured bytes"
                    )
                extracted = extract_publisher_timestamps(body)
                if (citation.published_at, citation.updated_at) != extracted[:2]:
                    raise ValueError(
                        "publisher timestamp differs from captured metadata"
                    )
            if not citation.retrieval_engine:
                raise ValueError(
                    "authority capture lacks its production retrieval engine"
                )
    ...
    if not isinstance(plan, list) or len(plan) != len(IntelligenceKind):
        raise ValueError(
            "source plan must cover all five intelligence kinds exactly once"
        )
    ...
    for entry in plan:
        ...
        if (
            entry_id in plan_by_id
            or kind in plan_kinds
            or source_id in plan_source_ids
            or not entry_id
            or source_id not in by_id
        ):
            raise ValueError(
                "source plan entry has invalid or non-unique identity, kind, or source"
            )
        if entry.get("permitted_purposes") != [kind.value]:
            raise ValueError(
                "source plan purpose must exactly match its intelligence kind"
            )
        if entry.get("source_type") not in SOURCE_KIND_POLICY[kind]["source_types"]:
            raise ValueError(
                "source plan source type is not permitted for its intelligence kind"
            )
        if authority_contract and by_id[source_id].get("source_kind") != entry.get(
            "source_type"
        ):
            raise ValueError("source-kind classification differs from source plan")
        if (
            type(entry.get("freshness_days")) is not int
            or entry["freshness_days"] != FRESHNESS_DAYS[kind]
        ):
            raise ValueError("source plan freshness policy does not match claim kind")
        source = by_id[source_id]
        if (
            "source_content_sha256" in entry
        ...
        if (
            "raw_response_ref" in entry
            and entry.get("raw_response_ref") != source["raw_response_ref"]
        ):
        ...
        if has_byte_range and (
            type(entry.get("excerpt_byte_start")) is not int
            or type(entry.get("excerpt_byte_length")) is not int
            or entry["excerpt_byte_start"] < 0
            or entry["excerpt_byte_length"] <= 0
        ):
            raise ValueError("source plan byte range is invalid")
        excerpt_hash = entry.get("excerpt_sha256")
        if not isinstance(excerpt_hash, str) or not re.fullmatch(
            r"[0-9a-f]{64}", excerpt_hash
        ):
    ...
    if plan_kinds != set(IntelligenceKind) or plan_source_ids != set(by_id):
        raise ValueError(
            "source plan must cover every intelligence kind and source exactly once"
        )
    if authority_contract:
        company_entry = next(
            item for item in plan if item["kind"] == IntelligenceKind.COMPANY.value
        )
        health_entry = next(
            item
            for item in plan
            if item["kind"] == IntelligenceKind.OPERATIONAL_HEALTH.value
        )
        company_publisher = by_id[str(company_entry["source_id"])][
            "canonical_publisher"
        ]
        health_source = by_id[str(health_entry["source_id"])]
        if (
            health_source["canonical_publisher"] == company_publisher
            and health_entry["source_type"] == "independent_reporting"
        ):
            raise ValueError(
                "operational-health corroboration is not publisher-independent"
            )
    ...
    for claim in claims:
        ...
        if (
            PROTECTED_FIELDS.intersection(claim.keys())
            or claim.get("subject_type") == "private_person"
        ):
        ...
        if excerpt is not None:
            ...
            if not any(
                ...
                in cache.resolve(
                    by_id[source_id]["raw_response_ref"],
                    by_id[source_id]["content_sha256"],
                )
        ...
        source_body = cache.resolve(
            by_id[cited[0]]["raw_response_ref"], by_id[cited[0]]["content_sha256"]
        )
        if "excerpt_byte_start" in entry:
            start, length = entry["excerpt_byte_start"], entry["excerpt_byte_length"]
            if (
                length != len(excerpt_bytes)
                or source_body[start : start + length] != excerpt_bytes
            ):
                raise ValueError(
                    "claim excerpt does not match its declared captured byte range"
                )
        ...
        if (
            ...
            and not source.get("publisher_date_evidence")
        ):
            raise ValueError(f"{kind.value} claim lacks byte-resolvable publisher date")
        if temporal is None:
            if entry.get("requires_current") is True or kind in {
                IntelligenceKind.ROLE,
                IntelligenceKind.HIRING,
                IntelligenceKind.OPERATIONAL_HEALTH,
            }:
                raise ValueError(
                    f"freshness-sensitive {kind.value} claim lacks publisher time evidence"
                )
            ...
            if claim.get("observed_at") != temporal:
                raise ValueError(
                    "claim observation time must be publisher-provided time"
                )
        ...
        if freshness == "unknown":
            if temporal is not None or observed_value is not None:
                raise ValueError(
                    "unknown freshness may only represent absent publisher time"
                )
            continue
        observed = datetime.fromisoformat(
            str(observed_value or "").replace("Z", "+00:00")
        ).date()
        age = as_of - observed
        if observed > as_of:
        ...
        if entry.get("requires_current") is True and freshness != "current":
            raise ValueError(
                f"current {kind.value} claim has no temporally valid evidence"
            )
    for edge in edges:
        if (
            edge.get("from_claim_id") not in claim_ids
            or edge.get("to_claim_id") not in claim_ids
        ):
    ...
    if len(claims) != len(IntelligenceKind) or {
        IntelligenceKind(c["kind"]) for c in claims
    } != set(IntelligenceKind):
...
def _validate_portable_dossier(
    ...
    *,
    ...
    for row in sources:
        ...
        if (
            not citation.id
            or citation.id in source_ids
            or not 200 <= citation.status_code < 300
        ):
        ...
        if identity in identities or citation.content_sha256 in hashes:
            raise ValueError(
                "duplicate content must be represented by its original capture"
            )
        identities.add(identity)
        ...
        if not citation.source_kind or not citation.retrieval_engine:
            raise ValueError(
                "capture lacks authority classification or retrieval metadata"
            )
        if require_access:
            if citation.retrieval_engine not in {
                "scrapling-static",
                "scrapling-dynamic",
            }:
        ...
        if (
            not publisher
            or (publisher != host and not host.endswith("." + publisher))
            or not article
            or _canonical_public_url(article) != citation.url
        ):
            raise ValueError("capture lacks publisher-owned canonical provenance")
        extracted = extract_publisher_timestamps(body)
        if citation.publisher_date_evidence is not None:
            if (
                citation.publisher_date_evidence.encode() not in body
                or (citation.published_at, citation.updated_at) != extracted[:2]
            ):
    ...
    claims_by_kind = {
        str(row.get("kind")): row for row in claims if isinstance(row, Mapping)
    }
    ...
    for kind_value in required:
        ...
        if (
            state not in {"supported", "unknown", "abstained"}
            or claim.get("outcome") != state
        ):
            raise ValueError("invalid or inconsistent intelligence outcome")
        if (
            outcome.get("permitted_purposes") != [kind.value]
        ...
        if state != "supported":
            if (
                claim.get("classification") is not None
                or claim.get("source_ids") not in ([], None)
                or claim.get("citation_excerpt") is not None
                or claim.get("score_delta_bp", 0) != 0
            ):
                raise ValueError(
                    "unsupported intelligence cannot become a claim or score contribution"
                )
        ...
        excerpt_bytes = excerpt.encode("utf-8")
        start, length = (
            outcome.get("excerpt_byte_start"),
            outcome.get("excerpt_byte_length"),
        )
        if (
        ...
        if (
            outcome.get("source_content_sha256", source["content_sha256"])
        ...
        if (
            purpose != [kind.value]
            or outcome.get("source_type")
            not in SOURCE_KIND_POLICY[kind]["source_types"]
        ):
        ...
        if any(
            start < previous_end and previous_start < start + length
            for previous_start, previous_end in ranges
        ):
            raise ValueError(
                "one capture may support multiple kinds only through disjoint excerpts"
            )
        ranges.add(byte_range)
        if not _kind_relevant(
            kind,
            excerpt,
            {"source_type": outcome.get("source_type"), "permitted_purposes": purpose},
        ):
            raise ValueError(f"kind-irrelevant {kind.value} evidence")
        temporal = source.get("updated_at") or source.get("published_at")
        ...
        if temporal is not None:
            if semantics != "publisher_time" or claim.get("observed_at") != temporal:
                raise ValueError(
                    "supported outcome has invalid publisher-time semantics"
                )
            observed = datetime.fromisoformat(
                str(temporal).replace("Z", "+00:00")
            ).date()
            if observed > as_of or as_of - observed > timedelta(
            ...
            or semantics != "retrieval_snapshot"
        ):
            raise ValueError("freshness-sensitive outcome lacks publisher time")


def _public_page_excerpts(
    body: bytes, count: int | None = None
) -> list[tuple[str, str]]:
...
def build_reconnaissance_dossier(
    task: Any,
    citation: Citation | Sequence[Citation],
    cache: RawResponseCache,
    *,
    ...
    if source_plan is not None and any("outcome" in entry for entry in source_plan):
        return _build_portable_dossier(task, citations, cache, source_plan, as_of=as_of)
    if source_plan is None and len(citations) != len(IntelligenceKind):
        ...
        return _build_portable_dossier(task, citations, cache, plan, as_of=as_of)
    identities = {item.capture_identity for item in citations}
    if (
        len(identities) != len(citations)
        or len({item.url for item in citations}) != len(citations)
        or len({item.content_sha256 for item in citations}) != len(citations)
    ):
        raise ValueError(
            "duplicate bodies and URL aliases cannot provide independent evidence"
        )
    paragraphs_by_source = {
        item.id: _public_page_excerpts(
            cache.resolve(item.raw_response_ref, item.content_sha256)
        )
        for item in citations
    }
    ...
    specifications = (
        ...
        (
            ...
            IntelligenceKind.HIRING,
            "hypothesis",
            f"Hiring intelligence for {role}",
            "official_careers",
        ),
    )
        ...
        source_plan = [
            {
                ...
                "kind": kind.value,
                "source_id": next(
                    item.id for item in citations if item.id.endswith(f":{kind.value}")
                ),
                "source_type": source_type,
                "permitted_purposes": [kind.value],
                "freshness_days": FRESHNESS_DAYS[kind],
    ...
    plan_by_kind = {
        IntelligenceKind(entry.get("kind")): dict(entry) for entry in source_plan
    }
    if set(plan_by_kind) != set(IntelligenceKind) or len(source_plan) != len(
        plan_by_kind
    ):
    ...
    for claim_id, kind, classification, _label, _ in specifications:
        ...
        eligible = [
            (excerpt, summary)
            for excerpt, summary in paragraphs_by_source[entry["source_id"]]
        ...
        if len(eligible) != 1:
            raise ValueError(
                f"ambiguous or missing {kind.value} evidence: {len(eligible)} eligible excerpts"
            )
        excerpt, summary = eligible[0]
        excerpt_bytes = excerpt.encode("utf-8")
        cited_source = next(item for item in citations if item.id == entry["source_id"])
        source_body = cache.resolve(
            cited_source.raw_response_ref, cited_source.content_sha256
        )
        ...
        if claim_observed is None and kind in {
            IntelligenceKind.ROLE,
            IntelligenceKind.HIRING,
            IntelligenceKind.OPERATIONAL_HEALTH,
        }:
        ...
        if claim_observed is None:
            ...
            evidence_date = datetime.fromisoformat(
                claim_observed.replace("Z", "+00:00")
            ).date()
            capture_date = datetime.fromisoformat(
                cited_source.captured_at.replace("Z", "+00:00")
            ).date()
            freshness = (
                "current"
                if capture_date - evidence_date <= timedelta(days=FRESHNESS_DAYS[kind])
                else "historical"
            )
        if entry.get("requires_current") is True and freshness != "current":
            raise ValueError(
                f"current {kind.value} claim has no temporally valid authoritative evidence"
            )
        claim.update(
            {
                "observed_at": claim_observed,
                "source_captured_at": cited_source.captured_at,
    ...
    dossier = {
        ...
        "edges": [
            {
                "from_claim_id": "company-fact",
                "to_claim_id": "product-inference",
                "relation": "supports",
            },
            {
                "from_claim_id": "company-fact",
                "to_claim_id": "health-inference",
                "relation": "qualifies",
            },
            {
                "from_claim_id": "product-inference",
                "to_claim_id": "role-hypothesis",
                "relation": "supports",
            },
            {
                "from_claim_id": "company-fact",
                "to_claim_id": "hiring-hypothesis",
                "relation": "qualifies",
            },
        ],
    }
    validate_dossier(dossier, cache, as_of=as_of)
    return dossier


def _build_portable_dossier(
    task: Any,
    citations: Sequence[Citation],
    cache: RawResponseCache,
    source_plan: Sequence[Mapping[str, Any]],
    *,
    as_of: date | None = None,
) -> dict[str, Any]:
    ...
    for entry in plan:
        ...
        if state in {"unknown", "abstained"}:
            ...
            continue
        source = next(
            (item for item in citations if item.id == entry.get("source_id")), None
        )
        if source is None:
        ...
        start, length = (
            int(entry["excerpt_byte_start"]),
            int(entry["excerpt_byte_length"]),
        )
    ...
    def run_once(self) -> str | None:
        ...
        try:
            if not hasattr(self.retriever, "retrieve_plan"):
                ...
                self.retriever = PortableAuthorityRetriever(
                    self.cache, retriever=self.retriever
                )
            ...
            validate_dossier(dossier, self.cache, as_of=self.as_of)
            ...
            self.database.record_research_failure(
                job_key=task.job_key,
                worker_id=self.worker_id,
...
class Opportunity1Coordinator:
    ...
    def __init__(
        self,
        database: Any,
        worker: EmployerResearchWorker,
        *,
        signal_deriver: Callable[[Mapping[str, Any]], Sequence[Mapping[str, Any]]]
        | None = None,
    ) -> None:
    ...
    def run_once(self) -> dict[str, Any] | None:
        ...
        if any(
            not isinstance(claim.get("citation_excerpt"), str)
            or not claim["citation_excerpt"].strip()
        ...
        if any(
            not isinstance(signal, Mapping) or signal.get("claim_id") not in claim_ids
            for signal in signals
        ):
            raise ValueError(
                "Opportunity-1 signals must resolve to completed dossier claims"
            )
        ...
        for signal in signals:
            ...
            grounded_text = " ".join(
                (str(claim.get("text", "")), str(claim.get("citation_excerpt", "")))
            ).casefold()
            meaningful = {
                word
                for word in re.findall(r"[a-z0-9]+", reason)
                if len(word) > 3
                and word not in {"with", "from", "that", "this", "were", "been"}
            }
            if (
                not reason
                or not meaningful
                or not meaningful.intersection(re.findall(r"[a-z0-9]+", grounded_text))
            ):
                raise ValueError(
                    "Opportunity-1 reason is not grounded in cited claim evidence"
                )
...
def load_frozen_dossiers(
    ...
    *,
    ...
    if strict_corpus and (
        ...
        raise ValueError(
            "certified JAA-04 corpus requires v5 access-policy-bound evidence"
        )
    ...
    for dossier in dossiers:
        ...
        if strict_corpus:
            for source in dossier["sources"]:
                ...
                if prior != identity:
                    raise ValueError(
                        "certified corpus has inconsistent robots bytes for one host"
                    )
        if (
            strict_corpus
            and {str(claim.get("kind")) for claim in dossier.get("claims", [])}
            != required_kinds
        ):
            raise ValueError("each frozen dossier must cover every intelligence kind")
        portable = dossier.get("schema_version") in {
            "jaa04.dossier.v3",
            "jaa04.dossier.v4",
        }
        if (
            ...
            and not portable
            and {
                str(claim.get("classification")) for claim in dossier.get("claims", [])
            }
            != {
                "fact",
                "inference",
            ...
            raise ValueError(
                "each frozen dossier must distinguish fact, inference, and hypothesis"
            )
        if (
            strict_corpus
            and not portable
            ...
            raise ValueError(
                "each frozen dossier requires a typed qualification or contradiction"
            )
        ...
        for claim in dossier["claims"]:
            ...
            if (
                strict_corpus
                and supported
                and (not isinstance(excerpt, str) or not excerpt.strip())
            ):
            ...
            if (
                strict_corpus
                and not portable
                and claim.get("freshness_classification")
                not in {"current", "historical"}
            ):
    ...
    if (
        ...
        raise ValueError(
            "frozen corpus must distinguish facts, inferences, and hypotheses"
        )
...
def ingest_frozen_manifest(
    ...
    cache: RawResponseCache,
    *,
    enforce_reviewed_bytes: bool = False,
) -> list[dict[str, Any]]:
    """Ingest the reviewed JAA-04 set through the production Scrapling path."""
    envelope = json.loads(Path(path).read_text(encoding="utf-8"))
    records = envelope.get("records")
    if (
        envelope.get("schema_version") != "jaa04.research-manifest.v1"
    ...
    for record in records:
        citation = retriever.retrieve(
            f"source:{record['id']}",
            record["url"],
            captured_at=record.get("captured_at", observed_at),
        )
        expected_digest = record.get("content_sha256")
        if (
            enforce_reviewed_bytes
            and expected_digest is not None
            and citation.content_sha256 != expected_digest
        ):
            raise ValueError("retrieved source differs from reviewed frozen bytes")
        source_plan = record.get("source_plan")
        if not isinstance(source_plan, list) or len(source_plan) != len(
            IntelligenceKind
        ):
        ...
        citations = [
            Citation(
                ...
                raw_response_ref=citation.raw_response_ref,
