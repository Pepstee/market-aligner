...
class FinalClickRevalidationError(ConsequentialActionError):
    """The in-primitive checks proved that no final click was attempted."""


def certified_final_submit_click(
    locator: Locator,
    authority: "ReleaseExecutionAuthority",
    *,
    verified_at: datetime,
    immediate_revalidation: Callable[[], None] | None = None,
) -> None:
    """The sole click primitive, with all three gates immediately rebound."""
    try:
        authority.verify_employer_facing_receipts(verified_at=verified_at)
        if authority.ats_provider == "greenhouse" and immediate_revalidation is None:
            raise ConsequentialActionError(
                "production submit requires in-primitive browser revalidation"
            )
        if immediate_revalidation is not None:
            immediate_revalidation()
    except Exception as exc:
        raise FinalClickRevalidationError(
            "final-click revalidation rejected before click dispatch"
        ) from exc
    locator.click()
...
class GreenhouseSuccessEvidence:
    """Observed positive provider semantics required after a submit click."""

    observation_sha256: str
    observed_at: str
    confirmation_url: str
    required_visible_markers: tuple[str, ...]
    schema_version: str = "jaa.greenhouse-success-evidence.v1"

    def __post_init__(self) -> None:
        try:
            observed = datetime.fromisoformat(self.observed_at.replace("Z", "+00:00"))
        except (AttributeError, ValueError) as exc:
            raise ValueError("Greenhouse success evidence is invalid") from exc
        if (
            self.schema_version != "jaa.greenhouse-success-evidence.v1"
            or not re.fullmatch(r"[0-9a-f]{64}", self.observation_sha256)
            or observed.tzinfo is None
            or observed.utcoffset() is None
            or not self.required_visible_markers
            or len(set(self.required_visible_markers))
            != len(self.required_visible_markers)
            or any(
                not marker or marker != marker.strip()
                for marker in self.required_visible_markers
            )
        ):
            raise ValueError("Greenhouse success evidence is invalid")
    ...
    def document(self) -> dict[str, object]:
        return {
            ...
            "required_visible_markers": self.required_visible_markers,
        }


def validate_greenhouse_success_observation(
    value: bytes,
    ...
    *,
    ...
    try:
        ...
        raise ValueError("provider success observation is invalid JSON") from exc
    ...
    try:
        ...
        raise ValueError("provider success observation time is invalid") from exc
    if (
        observed_at.tzinfo is None
        or evidence_at.tzinfo is None
        or verified_at.tzinfo is None
        or observed_at.astimezone(timezone.utc) != evidence_at.astimezone(timezone.utc)
    ):
        raise ValueError("provider success observation time is inconsistent")
    age = verified_at.astimezone(timezone.utc) - observed_at.astimezone(timezone.utc)
    if age < timedelta(minutes=-5) or age > timedelta(days=30):
        raise ValueError("provider success observation is stale")
    request = document.get("request")
    paths = document.get("provider_loader_paths")
    interaction = document.get("interaction")
    if (
        not isinstance(request, Mapping)
        or request.get("method") != "GET"
        or not isinstance(request.get("status"), int)
    ...
    if _canonical_https_route(observed_url) != _canonical_https_route(application_url):
        raise ValueError("provider success observation cites another vacancy route")
    ...
    if _canonical_https_route(observed_confirmation) != _canonical_https_route(
        ...
        raise ValueError("provider confirmation route was not observed")
    submit = urlsplit(str(submit_path))
...
def _canonical_https_route(value: str) -> str:
    ...
    if (
        ...
        raise ValueError("provider observation route is not canonical HTTPS")
    return f"https://{parsed.netloc.casefold()}{parsed.path.rstrip('/')}"
...
class ReleaseExecutionAuthority:
    ...
    sanity_review_receipt: SanityReviewReceipt = field(repr=False)
    archive_receipt: ApplicationArchiveReceipt = field(repr=False)
    archive_root: Path
    artifact_root: Path
    ...
    def __post_init__(self) -> None:
        ...
        if self.ats_provider == "jaa_loopback":
            if not _loopback_url(self.application_url) or not _loopback_url(
                ...
                raise ValueError("fixture release URLs must be loopback HTTP")
            if (
            ...
            if self.archive_receipt.vacancy.source_url != self.application_url:
                raise ValueError(
                    "Greenhouse application URL differs from the release archive"
                )
            if (
                not isinstance(self.success_evidence, GreenhouseSuccessEvidence)
                or self.success_evidence.confirmation_url != self.receipt_url
        ...
        if (
            not self.attached_roles
            or len(set(self.attached_roles)) != len(self.attached_roles)
            or "cv" not in self.attached_roles
            or not set(self.attached_roles) <= {"cv", "cover_letter"}
        ):
            raise ValueError("release authority attachment roles are invalid")
        if (
        ...
        intended_vacancy = IntendedVacancy(
            ...
            role_title=self.source.role_title,
        ...
        if (
            ...
            raise ValueError(
                "release authority requires exact CV and cover-letter assurance"
            )
        verify_receipt_for_pdf(
            self.document_assurance_receipts[0],
            self.artifacts.cv_pdf.pdf_bytes,
            intended_vacancy=intended_vacancy,
        )
        verify_sanity_review_receipt(
            self.sanity_review_receipt,
            self.sanity_review_package(),
        )
        verify_receipt_for_pdf(
            self.document_assurance_receipts[1],
    ...
    def verify_employer_facing_receipts(
        ...
        """Recheck exact PDFs, semantic sanity, and archive as one boundary."""
        intended_vacancy = IntendedVacancy(
            job_key=self.source.job_key,
            vacancy_sha256=self.source.vacancy_sha256,
            role_title=self.source.role_title,
        ...
        current_assurance = assert_application_artifacts(
            ...
            answers_text=self.artifacts.editable.answers_text,
            intended_vacancy=intended_vacancy,
        )
        if current_assurance != self.document_assurance_receipts:
            raise ReleaseGateError(
                "submit lacks exact vacancy-bound document assurance"
            )
        verify_receipt_for_pdf(
            self.document_assurance_receipts[0],
            self.artifacts.cv_pdf.pdf_bytes,
        ...
        self.verify_archive_receipt(verified_at=verified_at)

    def verify_archive_receipt(self, *, verified_at: datetime | None = None) -> None:
        """Rebind the durable release archive to every employer-facing byte."""
        if not isinstance(self.archive_receipt, ApplicationArchiveReceipt):
            raise TypeError("release authority requires an application archive receipt")
        verification_time = verified_at or self.consumed_at
        provider_authority = None
        if self.ats_provider == "greenhouse" and self.success_evidence is not None:
            observation = selected_archive_object_bytes(
        ...
        expected_selected = release_authority_selected_sha256(
            ...
            cover_letter_pdf_bytes=self.artifacts.cover_letter_pdf.pdf_bytes,
            answers_text=self.artifacts.editable.answers_text,
            cv_assurance_document=self.document_assurance_receipts[0].document(),
            cover_letter_assurance_document=(
                self.document_assurance_receipts[1].document()
            ),
            ...
            provider_success_observation_sha256=(
                self.success_evidence.observation_sha256
                if self.success_evidence is not None
                else None
            ),
        ...
        verify_application_archive_receipt(
            ...
            root=self.archive_root,
            repository_root=self.repository_root,
            expected_vacancy=expected_vacancy,
...
def _validate_greenhouse_routes(
    application_url: str,
    receipt_url: str,
    application_id: str,
) -> None:
    ...
    if (
        ...
        or application.hostname not in allowed_hosts
...
class LocalBrowserExecutor:
    ...
    def _execute_submit(
        ...
        action = pending.action
        if authority.ats_provider != "jaa_loopback":
            raise ReleaseGateError(
                "local fixture executor cannot run a production ATS authority"
            )
        ...
        if current_assurance != authority.document_assurance_receipts:
            raise ReleaseGateError(
                "submit lacks exact vacancy-bound document assurance"
            )
        # Semantic authority is independently rebound at the last possible
        # moment, after exact deterministic PDF assurance and before dispatch.
        verify_sanity_review_receipt(
            authority.sanity_review_receipt,
            authority.sanity_review_package(),
        )
        authority.verify_archive_receipt()
        ...
        if state == "release_consumed":
            ...
            # This is the final filesystem read before recording click intent
            # and invoking the one consequential browser operation.
            authority.verify_archive_receipt(verified_at=durable_consumed_at)
