...
CERTIFIED_INTEGRATION = "3d239d40ac7a74f6f390e231d967ed85dca4e4d7"
ALLOWLIST = {
...
def _chromium() -> Path:
    ...
    if package is None or package.origin is None:
        raise AssertionError("the pinned Playwright package is unavailable")
    browsers = (
        Path(package.origin).parent / "driver" / "package" / "browsers.json"
    )
    revision = next(
        str(row["revision"])
        for row in json.loads(browsers.read_text())["browsers"]
...
def test_historical_fable_integration_allowlist_remains_in_lineage() -> None:
    ...
    changed = set(
        subprocess.run(
            ["git", "diff", "--name-only", DESIGN_BASE, CERTIFIED_INTEGRATION],
            cwd=ROOT,
            check=True,
            capture_output=True,
            text=True,
    ...
    subprocess.run(
        ["git", "merge-base", "--is-ancestor", CERTIFIED_INTEGRATION, "HEAD"],
        cwd=ROOT,
        check=True,
    )
