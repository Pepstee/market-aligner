...
class PublishedArtifactReceipt:
    ...
    def __post_init__(self) -> None:
        ...
        if (
            tuple(row.filename for row in self.files) != ARTIFACT_FILENAMES
            or len({row.filename for row in self.files}) != len(self.files)
        ):
...
def _read_receipt(directory: Path) -> PublishedArtifactReceipt:
    ...
    if (
        ...
        or canonical_json(document) != canonical_json(receipt.document())
    ):
    ...
    for row in receipt.files:
        ...
        if len(value) != row.size_bytes or hashlib.sha256(value).hexdigest() != row.sha256:
