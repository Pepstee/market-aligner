...
LOCKED_PACKS = ROOT / "career_automation/fixtures/jaa07_locked_application_packs.json"
...
def test_pdf_artifacts_are_compact_cv_one_page_letter_and_parse_exactly() -> None:
    source, _ = _source()
    ...
    assert (
        ...
        == render_pdf_artifacts(source).artifact_set_sha256
    )
...
def test_artifacts_publish_when_operator_contact_has_no_phone(tmp_path: Path) -> None:
    source, _ = _source()
    source = replace(
        source,
        contact=replace(source.contact, phone=None),
        source_id="0" * 64,
        content_sha256="0" * 64,
    )
    content_sha256 = hashlib.sha256(
        canonical_json(source.document(include_identity=False)).encode()
    ).hexdigest()
    source = replace(
        source,
    ...
    receipt = publish_application_artifacts(
        source,
        artifacts,
        root=tmp_path / "phone-absent-artifacts",
        repository_root=ROOT,
    )
    assert receipt.artifact_set_sha256 == artifacts.artifact_set_sha256
    assert "+44 7700 900123" not in artifacts.cv_pdf.extracted_text
...
def test_production_compiler_covers_multiple_requirements_sections_and_cv_pages(
    ...
    artifacts = render_pdf_artifacts(source)
    assert artifacts.cv_pdf.page_count == 1
    (page,) = artifacts.cv_pdf.rendered_lines
    assert "Education" in page
    assert "Experience" in page
    assert "Skills" in page
    for requirement_id in requirement_ids:
        assert any(
            row.authority.requirement_id == requirement_id and row.document_kind == "cv"
...
def test_twenty_locked_packs_pass_exact_noncertifying_software_metrics() -> None:
    ...
    assert all(set(row["checks"].values()) == {True} for row in report["results"])
