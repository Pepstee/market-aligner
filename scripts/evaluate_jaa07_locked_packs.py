...
def _pack(case: Mapping[str, object]) -> ApplicationSource:
    ...
    employer_text = f"{company} publishes an official service reliability objective."
    employer_document = {
        "id": research_id,
        "kind": "company",
        "classification": "fact",
...
def evaluate(fixture: Mapping[str, object]) -> dict[str, object]:
    if (
        ...
        or tuple(fixture.get("dependency_gates", ())) != ("JAA-06", "JAA-05", "JAA-04")
    ...
    for case in cases:
        ...
        page_ok = (
            artifacts.cv_pdf.page_count in {1, 2}
        ...
        section_ok = headings == ("Professional Summary", "Experience") and tuple(
            row.heading for row in source.letter_sections
        ) == ("Opening", "Evidence Match", "Company Fit", "Close")
        requirements = {row.authority.requirement_id for row in source.facts}
        coverage_ok = len(requirements) == 1 and all(
            requirements
            == {
        ...
        provenance_ok = all(
            row.text == row.approved_source_text and row.authority.strategy_element_id
            for row in source.facts
        )
        combined = (
        ...
        if case.get("expected_artifact_set_sha256") != artifacts.artifact_set_sha256:
            replay_ok = False
        checks = {
    ...
    metrics = {name: passed[name] * 10_000 // len(cases) for name in METRIC_NAMES}
    ...
    return {
        ...
        "fixture_sha256": hashlib.sha256(canonical_json(fixture).encode()).hexdigest(),
        "packs": len(cases),
        "metrics_bp": metrics,
        "results": results,
