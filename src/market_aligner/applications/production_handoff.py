...
def _instant(value: object, label: str) -> datetime:
    if not isinstance(value, str) or not value.endswith(("+00:00", "Z")):
        raise ProductionHandoffError("invalid_timestamp", f"{label} must be explicit UTC")
    try:
        parsed = datetime.fromisoformat(value[:-1] + "+00:00" if value.endswith("Z") else value)
    except ValueError as exc:
        raise ProductionHandoffError("invalid_timestamp", f"{label} is invalid") from exc
...
def _read_regular(path: Path, label: str, *, private: bool = False) -> bytes:
    parent_fd = _open_directory_chain(path.parent.absolute(), require_private_leaf=private)
    try:
        descriptor = os.open(
            path.name,
    ...
    try:
        ...
        if (
            ...
            raise ProductionHandoffError("unsafe_input", f"{label} is not a protected regular file")
        chunks: list[bytes] = []
        while True:
...
class _SecureArchive:
    ...
    def __init__(self, data_home: Path, identity: str) -> None:
        ...
        if relative.is_absolute() or not relative.parts or ".." in relative.parts:
            raise ProductionHandoffError("research_archive", "store archive identity is unsafe")
        descriptor = _open_directory_chain(data_home.absolute(), require_private_leaf=True)
        try:
            for component in relative.parts:
                ...
                if metadata.st_uid != os.geteuid() or stat.S_IMODE(metadata.st_mode) & 0o077:
                    ...
                    raise ProductionHandoffError("research_archive", "archive directory is not private")
                os.close(descriptor)
                descriptor = next_descriptor
    ...
    def read(self, category: str, name: str) -> bytes:
        if category not in {"metadata", "objects", "receipts"} or "/" in name or name in {"", ".", ".."}:
            raise ProductionHandoffError("research_archive", "archive component is invalid")
        directory = os.open(
            category,
            os.O_RDONLY | os.O_DIRECTORY | os.O_CLOEXEC | getattr(os, "O_NOFOLLOW", 0),
        ...
        try:
            ...
            try:
                ...
                if (
                    ...
                    raise ProductionHandoffError("research_archive", "archive object is not private")
                chunks: list[bytes] = []
                while True:
                    chunk = os.read(descriptor, 1_048_576)
                    if not chunk:
...
def _persist_execution_receipt(root: Path, semantic_sha256: str, exact: bytes) -> Path:
    ...
    try:
        ...
        try:
            ...
            try:
                descriptor = os.open(name, flags, 0o600, dir_fd=receipts_fd)
            except FileExistsError:
                descriptor = os.open(
                    name,
                    os.O_RDONLY | os.O_CLOEXEC | getattr(os, "O_NOFOLLOW", 0),
                ...
                try:
                    ...
                    if (
                        ...
                        raise ProductionHandoffError(
                            "execution_receipt", "stored receipt is not private"
                        )
                    existing = b""
                    while True:
                        chunk = os.read(descriptor, 1_048_576)
                        ...
                        existing += chunk
                    if existing != exact:
                        raise ProductionHandoffError("execution_receipt_replay", "stored receipt differs")
                finally:
                    os.close(descriptor)
            else:
                try:
                    ...
                    while view:
                        ...
                        if written <= 0:
                            raise ProductionHandoffError(
                                "execution_receipt", "receipt write did not progress"
                            )
                        view = view[written:]
                    os.fsync(descriptor)
                finally:
...
def _document(raw: bytes, label: str) -> dict[str, Any]:
    try:
        ...
        raise ProductionHandoffError("invalid_input", f"{label} is invalid JSON") from exc
...
def _git_commit(repository_root: Path) -> str:
    ...
    if status.stdout:
        raise ProductionHandoffError(
            "producer_dirty", "production handoff requires a completely clean repository"
        )
...
def _workable_identity(url: str) -> tuple[str | None, str]:
    ...
    if parsed.scheme != "https" or parsed.hostname != "apply.workable.com":
        raise ProductionHandoffError("official_route", "official observation is not Workable")
    ...
    if len(segments) == 2 and segments[0] == "j":
        ...
        raise ProductionHandoffError("official_route", "Workable vacancy route is not canonical")
    if not re.fullmatch(r"[A-Z0-9]{10}", vacancy_id):
        raise ProductionHandoffError("official_route", "Workable vacancy identity is malformed")
    try:
        port = parsed.port
    except ValueError as exc:
        raise ProductionHandoffError("official_route", "Workable URL port is malformed") from exc
    if parsed.query or parsed.fragment or parsed.username or parsed.password or port not in {None, 443}:
        raise ProductionHandoffError("official_route", "Workable vacancy URL has unsupported authority")
...
def _validated_dossier(document: Mapping[str, Any]) -> ResearchDossier:
    try:
        citations = tuple(SourceCitation(**row) for row in document.get("citations", []))
        claims = tuple(
            ResearchClaim(
                claim=row["claim"],
                citation_ids=tuple(row["citation_ids"]),
                confidence=float(row["confidence"]),
                supports=tuple(ClaimSupport(**support) for support in row.get("supports", [])),
            )
        ...
        dossier = ResearchDossier(
            ...
            canonical_vacancy_object_sha256=document.get("canonical_vacancy_object_sha256"),
            schema_version=document.get("schema_version", ""),
        )
        dossier.validate()
        return dossier
    except (KeyError, TypeError, ValueError) as exc:
        raise ProductionHandoffError("research_dossier", "v2 dossier contract is invalid") from exc
    ...
    *,
    ...
    try:
        binding = ResearchEvidenceBinding(
            ...
            vacancy_snapshot_sha256=str(evidence_row.get("vacancy_snapshot_sha256") or ""),
            promotion_receipt_sha256=str(evidence_row.get("promotion_receipt_sha256") or ""),
            canonical_vacancy_object_sha256=str(
                evidence_row.get("canonical_vacancy_object_sha256") or ""
            ),
            semantic_receipt_sha256=str(evidence_row.get("semantic_receipt_sha256") or ""),
    ...
    if (
        dossier.canonical_vacancy_object_sha256 != binding.canonical_vacancy_object_sha256
        ...
        raise ProductionHandoffError("research_binding", "v2 research differs from current vacancy")
    ...
    if (
        receipt.get("schema_version") != "market-aligner.public-research-materialization.v2"
    ...
    citations = [row for row in dossier_document.get("citations", []) if row.get("source_kind") == "canonical_vacancy"]
    if len(citations) != 1:
        raise ProductionHandoffError("research_dossier", "canonical vacancy citation is absent")
    citation = citations[0]
    object_sha = evidence_row.get("canonical_vacancy_object_sha256")
    if citation.get("content_sha256") != object_sha or citation.get("url") != canonical_url:
        raise ProductionHandoffError("research_dossier", "canonical vacancy citation differs")
    matching = [
        row for row in receipt.get("entries", []) if isinstance(row, dict)
        and row.get("citation_id") == citation.get("citation_id")
        and row.get("object_sha256") == object_sha
    ]
    if len(matching) != 1:
        raise ProductionHandoffError("research_receipt", "canonical vacancy entry is absent")
    metadata_sha = matching[0].get("metadata_sha256")
    if not isinstance(metadata_sha, str) or not _SHA256.fullmatch(metadata_sha):
        raise ProductionHandoffError("research_metadata", "metadata identity is invalid")
    ...
    if (
        ...
        raise ProductionHandoffError("research_metadata", "canonical vacancy metadata differs")
    ...
    if (
        ...
        raise ProductionHandoffError("research_object", "canonical vacancy object differs")
    entry_by_citation = {
        row["citation_id"]: row for row in receipt.get("entries", []) if isinstance(row, dict)
    }
    ...
    for claim in dossier_document["claims"]:
        for support in claim["supports"]:
            ...
            if source is None or entry is None or entry.get("object_sha256") != source.get("content_sha256"):
                archive.close()
                raise ProductionHandoffError("research_support", "claim source binding differs")
            ...
            match = re.fullmatch(r"bytes:(0|[1-9][0-9]*)-(0|[1-9][0-9]*)", support["selector"])
            if match is None:
                archive.close()
                raise ProductionHandoffError("research_support", "claim selector is invalid")
            ...
            try:
                ...
                archive.close()
                raise ProductionHandoffError("research_support", "claim excerpt is not UTF-8") from exc
            if (
                ...
                archive.close()
                raise ProductionHandoffError("research_support", "claim is not exact archived text")
    ...
    if age < -300 or age > maximum_age_seconds:
        archive.close()
        raise ProductionHandoffError("official_source_stale", "official source is not current")
    archive.close()
...
def _build_production_handoff_from_authenticated_time(
    *,
    ...
    if vacancy_maximum_age_seconds <= 0 or dossier_maximum_age_seconds <= 0:
        raise ProductionHandoffError("freshness_policy", "freshness intervals must be positive")
    ...
    if track not in profile.tracks:
        raise ProductionHandoffError("candidate_track", "profile does not authorize this track")
    ...
    try:
        promotion_row = service.assessments.processing_promotion(profile_id, source_job_key)
    ...
    if promotion_row["track"] != track:
        raise ProductionHandoffError(
            "processing_promotion", "canonical assessment promotion targets another track"
        )
    ...
    if posting is None or len(processing) != 1 or posting["fetch_status"] != "fetched":
        raise ProductionHandoffError("vacancy_state", "current fetched processing row is absent")
    ...
    if (
        ...
        raise ProductionHandoffError("vacancy_hash", "current vacancy source bytes differ")
    result = _document(str(processing[0]["result_json"]).encode(), "processing result")
    vacancy = result.get("vacancy")
    if not isinstance(vacancy, dict) or vacancy.get("source_content_sha256") != posting[
        "content_hash"
    ]:
    ...
    if _sha(dossier_bytes) != dossier_row["dossier_hash"]:
        raise ProductionHandoffError("employer_research_hash", "canonical dossier differs")
    ...
    if (
        ...
        raise ProductionHandoffError("employer_research_binding", "canonical dossier differs")
    try:
        dossier_issued = datetime.fromisoformat(
    ...
    if (current - dossier_issued).total_seconds() > dossier_maximum_age_seconds:
        raise ProductionHandoffError("employer_research_stale", "canonical dossier is stale")

    metadata_bytes, source_object_bytes, source_receipt_bytes, observed_at = _research_evidence(
    ...
    if any(
        ...
        raise ProductionHandoffError("eligibility_state", "processing hard gate did not pass")
    ...
    if _sha(selection_policy_bytes) != str(promotion_row["policy_hash"]):
        raise ProductionHandoffError("selection_policy", "promotion policy hash differs")
    ...
    receipt_bytes = _canonical({**receipt_basis, "semantic_receipt_sha256": receipt_semantic_sha})
